...
export const partnerTypes = ["realtor", "mortgage_broker", "lender", "lawyer", "accountant", "property_manager", "contractor", "appraiser", "inspector", "architect", "urban_planner", "other"] as const;
...
export const realtorLeadNotifications = pgTable("realtor_lead_notifications", {
  ...
  propertyId: varchar("property_id").references(() => properties.id),
  analysisId: varchar("analysis_id").references(() => analyses.id),
  // Discriminator: financing-intent leads are routed to mortgage_broker/lender
  // claims through this same table instead of a parallel notifications table,
  // so claim/introduction/referral-outcome flows stay shared.
  partnerType: text("partner_type").default("realtor").notNull(),
...
// ============================================
// JV PARTNER MATCHING SCHEMAS
// ============================================

export const jvPartnerRoles = ["land", "capital", "development", "realtor", "gc"] as const;
export type JvPartnerRole = (typeof jvPartnerRoles)[number];

export const jvListingStatuses = ["active", "closed"] as const;
export type JvListingStatus = (typeof jvListingStatuses)[number];

export const jvMatchTypes = ["auto", "interest"] as const;
export type JvMatchType = (typeof jvMatchTypes)[number];

export const jvMatchStatuses = ["suggested", "interested", "accepted", "declined"] as const;
export type JvMatchStatus = (typeof jvMatchStatuses)[number];

// A JV partner listing: what role a user is offering (or bringing) to a deal
export const jvPartnerListings = pgTable("jv_partner_listings", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  userId: varchar("user_id").references(() => users.id).notNull(),
  title: text("title").notNull(),
  description: text("description"),
  partnerRoleOffered: text("partner_role_offered").notNull(),
  city: text("city"),
  province: text("province"),
  investmentMin: real("investment_min"),
  investmentMax: real("investment_max"),
  status: text("status").default("active").notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});

// Matches are stored explicitly so users can manage them in the portal.
// matchType "auto" rows are generated by the matching algorithm on listing
// creation; "interest" rows are created when a user expresses interest.
export const jvPartnerMatches = pgTable("jv_partner_matches", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  listingId: varchar("listing_id").references(() => jvPartnerListings.id).notNull(),
  matchedUserId: varchar("matched_user_id").references(() => users.id).notNull(),
  matchType: text("match_type").default("auto").notNull(),
  status: text("status").default("suggested").notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
});

export const jvPartnerListingsRelations = relations(jvPartnerListings, ({ one, many }) => ({
  owner: one(users, {
    fields: [jvPartnerListings.userId],
    references: [users.id],
  }),
  matches: many(jvPartnerMatches),
}));

export const jvPartnerMatchesRelations = relations(jvPartnerMatches, ({ one }) => ({
  listing: one(jvPartnerListings, {
    fields: [jvPartnerMatches.listingId],
    references: [jvPartnerListings.id],
  }),
  matchedUser: one(users, {
    fields: [jvPartnerMatches.matchedUserId],
    references: [users.id],
  }),
}));

export const insertJvPartnerListingSchema = createInsertSchema(jvPartnerListings).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export const insertJvPartnerMatchSchema = createInsertSchema(jvPartnerMatches).omit({
  id: true,
  createdAt: true,
});

export type InsertJvPartnerListing = z.infer<typeof insertJvPartnerListingSchema>;
export type JvPartnerListing = typeof jvPartnerListings.$inferSelect;

export type InsertJvPartnerMatch = z.infer<typeof insertJvPartnerMatchSchema>;
export type JvPartnerMatch = typeof jvPartnerMatches.$inferSelect;
