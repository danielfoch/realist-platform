/**
 * Buy & hold underwriting engine — the single source of truth for the web
 * analyzer (client/src/lib/calculations.ts re-exports this module), the agent
 * API / MCP underwriting tools, and the hosted result views at /v/:token.
 *
 * Pure math: no DOM, no network, no database. Moved out of the client so the
 * server can produce the same 10-year pro forma, IRR, and stress test the
 * website shows (docs/DISTRIBUTION_ARCHITECTURE.md, Track B2).
 */
import type { BuyHoldInputs, AnalysisResults } from "./schema";
