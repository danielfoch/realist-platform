...
- **DDF Yield Crawler**: Monthly automated crawl (`server/ddfYieldCrawler.ts`) of 33 Canadian cities across CREA DDF, computing gross/net yields per listing, storing snapshots to `ddf_listing_snapshots` and aggregated city-level yields to `city_yield_history`. Uses full province names for DDF RESO API (e.g., "Ontario" not "ON"), page size 100, yield bounds (gross 0-20%, net -10 to 15%). Runs monthly on the 1st via cron, admin manual trigger via `POST /api/ddf-crawl/trigger`.
