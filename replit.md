...
- **StatCan Labour Force Survey Reports**: SEO-indexable monthly recaps of Statistics Canada's LFS releases, framed for real-estate investors. First instance is the April 2026 LFS at `/insights/statcan-labour-force-survey-april-2026` (`LabourForceSurveyApril2026Report.tsx`): hero summary, four key takeaways, KPI grid, recharts visualizations (employment-rate trend, unemployment trend with reference line, provincial employment change, provincial unemployment, industry change), an 8-section narrative, a 5-tab Real Estate implications module (Buyers/Sellers/Investors/Renewers/Realtors), and a CTA strip. Sets `NewsArticle` JSON-LD structured data, sources both the StatCan HTML and PDF, and is surfaced first in the Canadian Macro Reports grid on `/insights/market-report`.
