...
export function ListingCard({ listing }: { listing: ListingSearchResult }) {
  ...
  // The save button sits beside the link, not inside it: a button nested in an
  // anchor is invalid and unreachable by keyboard.
  return (
    <article className="group relative flex flex-col overflow-hidden rounded-xl border border-hairline bg-surface transition-all hover:-translate-y-0.5 hover:border-brand/40 hover:shadow-md">
      <Link
        href={`/listings/${encodeURIComponent(listing.mlsNumber)}`}
        className="flex flex-1 flex-col focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-brand"
      >
        {photo ? (
