...
- Agent API jobs spine (P0–P5): `POST/GET /api/agent/jobs`, approve/cancel, `GET /api/agent/openapi.json`. Forms, listing extract, Realist CRM, browser playbooks, and `docs.route` (`GET /api/agent/docs/classes`, `POST /api/agent/docs/route`; always `needs_approval` before attachment metadata). Opt-in scopes `jobs:write`, `forms:write`, `docs:write`, `crm:write`, `browser:write`; defaults stay `read`/`underwrite`/`deal:submit`.

### Pending/Recent Work
- Agent API specialist spine P0–P5 — canonical schemas in `shared/agentSpine.ts`. Ontario/OREA field maps, worldwide listing extract, Realist CRM, browser playbooks, and document routing (`shared/docsRoute.ts`, `server/agentDocs.ts`). OpenAPI at `docs/openapi/agent-api.yaml`, plug-in guide in `docs/specialist-spine.md`. Realist-only (no Homies). No login/paywall scrape. No blank-form storage. No external CRM.
