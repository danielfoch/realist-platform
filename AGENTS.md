...
- OAuth 2.1 for `/mcp` (branch `feat/mcp-oauth`, stacked on the above): `server/agent/oauth/{store,provider,routes}.ts`, consent page `client/src/pages/OAuthConsent.tsx` (`/oauth/consent`, noindex, never framable), "Connected apps" on `/account/api-keys`. Endpoints: `/.well-known/oauth-protected-resource[/mcp]`, `/.well-known/oauth-authorization-server`, `/oauth/{register,authorize,token,revoke}` (MCP SDK handlers), `/api/oauth/requests/:id[/approve|/deny]`, `/api/oauth/grants[/:id]`. New tables (schema.ts + idempotent boot DDL, all secrets SHA-256 hashed): `oauth_clients`, `oauth_authorization_requests`, `oauth_authorization_codes`, `oauth_grants`, `oauth_tokens`. `/mcp` now accepts an API key OR an OAuth access token (`realist_oat_…`); REST stays API-key only.
