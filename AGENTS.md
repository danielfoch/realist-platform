...
- Agent API jobs spine (P0+P1+P2): `POST/GET /api/agent/jobs`, `GET /api/agent/jobs/:id`, `POST /api/agent/jobs/:id/approve`, `POST /api/agent/jobs/:id/cancel`, `GET /api/agent/openapi.json`. Forms: `GET /api/agent/forms`, `POST /api/agent/forms/fill`. Worldwide listing ingest: `GET /api/agent/listings/extractors`, `POST /api/agent/listings/extract`, `POST /api/agent/listings/underwrite-url` (`listing.extract` + optional `underwrite.custom`). Existing `/api/agent/underwrite/*`, find-deals, and me routes unchanged. New opt-in scopes `jobs:write`, `forms:write`, `docs:write`, `crm:write`; defaults stay `read`/`underwrite`/`deal:submit`.

### Pending/Recent Work
- Agent API specialist spine P0 + P1 Forms + P2 listing extract — canonical schemas in `shared/agentSpine.ts` (ISO country/currency, optional MLS, sourceUrl). `agent_jobs` persistence. Ontario/OREA field maps + fill in `shared/forms/` (maps only; Ontario-only). Worldwide URL extract in `shared/listingExtract/` + `server/listingExtract.ts` (JSON-LD/OG/`__NEXT_DATA__`/`PAGE_MODEL`; realtor.ca, Zillow, Redfin, Realtor.com, Homes.com, Rightmove, Zoopla, Domain, realestate.com.au, ImmoScout, SeLoger, Idealista, PropertyGuru, CREA DDF MLS fast path, `registerExtractor`). OpenAPI at `docs/openapi/agent-api.yaml`, plug-in guide in `docs/specialist-spine.md` (“Zillow for Earth for AI agents”). Realist-only (no Homies). No login/paywall scrape.
