...
export function buildListingSeoDescription(listing: ListingSeoRecord): string {
  ...
  const facts = [
    ...
    toNumber(listing.netYield) ? `${toNumber(listing.netYield)?.toFixed(1)}% net yield` : null,
...
// primary photo are real columns on the snapshot.
const SNAPSHOT_SEO_SELECT = sql`
  SELECT
    s.id,
    s.mls_number AS "mlsNumber",
    COALESCE(NULLIF(s.standard_status, ''), 'Active') AS "status",
    CASE
    ...
    NULLIF(TRIM(split_part(s.street_address, ',', 1)), '') AS "addressStreet",
    NULL AS "addressUnit",
...
export async function getListingSeoByMls(mlsNumberRaw: string): Promise<ListingSeoRecord | null> {
  ...
  const result = await getDb().execute(sql`
    ${SNAPSHOT_SEO_SELECT}
    WHERE s.mls_number = ${mlsNumber}
    ORDER BY s.snapshot_month DESC, s.captured_at DESC
    LIMIT 1
  `);

  return (result.rows[0] as ListingSeoRecord | undefined) || null;
}
...
export async function getListingSitemapRecords(limit = 10000): Promise<ListingSitemapRecord[]> {
  ...
  const result = await getDb().execute(sql`
    SELECT "mlsNumber", "lastUpdated", "listDate" FROM (
      ...
      WHERE s.mls_number IS NOT NULL
        ...
        AND NULLIF(TRIM(split_part(s.street_address, ',', 1)), '') IS NOT NULL
  ...
  return result.rows as ListingSitemapRecord[];
}
