...
import { configReports, getConfigReport, sortedConfigReports } from "@/content/reports";
...
function baseReport(overrides: Partial<ReportContent> = {}): ReportContent {
  return {
    ...
    cta: { toolUrl: "/listings", headline: "Go", body: "Do it." },
    ...overrides,
  };
}
  ...
  it("accepts every section type together", () => {
    const report = baseReport({
      sections: [
        ...
        { type: "image", src: "/reports/test/fig.jpg", alt: "A figure.", width: 800, height: 600 },
...
describe("validateReportContent — rejects bad reports", () => {
  ...
  it("rejects an image without alt text or with a bare filename src", () => {
    const result = validateReportContent(
      baseReport({
        sections: [{ type: "image", src: "fig.jpg", alt: " " }],
      }),
    );
    expect(result.valid).toBe(false);
    expect(result.errors.some((e) => e.includes("site-relative"))).toBe(true);
...
describe("seriesColor", () => {
  it("uses the explicit color when set", () => {
    expect(seriesColor({ key: "a", label: "A", color: "var(--color-bad)" }, 0)).toBe("var(--color-bad)");
  });
  it("falls back to the design-token palette by index, wrapping", () => {
    expect(seriesColor({ key: "a", label: "A" }, 0)).toBe("var(--color-brand)");
    expect(seriesColor({ key: "a", label: "A" }, 5)).toBe("var(--color-brand)");
  });
});
...
describe("routing + ordering", () => {
  it("reportRoute uses the /research prefix", () => {
    expect(CONFIG_REPORT_ROUTE_PREFIX).toBe("/research");
    expect(reportRoute("x")).toBe("/research/x");
  });

  it("sortReportsNewestFirst sorts by publishDate descending", () => {
...
describe("content dir (content/reports)", () => {
  ...
  it("sortedConfigReports is newest-first", () => {
    const dates = sortedConfigReports.map((r) => r.publishDate);
    const resorted = [...dates].sort((a, b) => b.localeCompare(a));
    expect(dates).toEqual(resorted);
  });
  ...
  it("getConfigReport resolves the OSFI image-led report", () => {
    const report = getConfigReport("osfi-dsb-june-2026");
    expect(report).toBeDefined();
    expect(report?.sections.some((s) => s.type === "image")).toBe(true);
  });
