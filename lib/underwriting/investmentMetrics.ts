...
/**
 * Listing-level yield snapshot for the DDF crawler. Maps a listing's observable
 * facts (price, monthly rent, annual tax, monthly condo/association fee) onto
 * the shared assumption model — insurance proxied at 0.3% of price, the
 * vacancy/maintenance/management defaults left to the engine — and reads
 * gross/net yield back out of the one engine instead of re-deriving them.
 * "Net yield" here is the engine's cap rate over estimated NOI; it is stored
 * as net_yield because the expense stack is largely inferred.
 */
export function calculateListingYield(
  listPrice: number,
  monthlyRent: number,
  taxAnnualAmount = 0,
  ...
  return {
    ...
    netYield: metrics.capRate ?? 0,
    estimatedExpenses: metrics.annualOperatingExpenses ?? 0,
    estimatedNoi: metrics.noi ?? 0,
  };
}
