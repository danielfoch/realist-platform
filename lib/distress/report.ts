...
import { distressReports, distressSnapshots } from "@/lib/db/schema";
import { searchDdfByRemarks, normalizeDdfListing, isDdfConfigured } from "@/lib/ddf/client";
import { SEARCH_TERMS_BY_CATEGORY, isQualifiedDistressResult, scoreDistress } from "./scoring";
...
export async function scanProvinceDistress(provinceName: string): Promise<ScoredListing[]> {
  const allTerms = [
...
function generateDistressReportHtml(params: {
  ...
  sections.push(`
    ...
    <p>Want to see the actual listings? Visit the <a href="/deals">Motivated Deals Browser</a> on Realist.ca to search, filter, and map motivated-seller listings in real time across all Canadian provinces.</p>
  `);
...
export async function generateDistressReport(month: string): Promise<{ created: boolean; slug: string; message: string }> {
  ...
  const db = getDb();
  const [existingReport] = await db.select({ slug: distressReports.slug })
    .from(distressReports).where(eq(distressReports.slug, slug));
  if (existingReport) {
  ...
  const html = generateDistressReportHtml({
  ...
  await db.insert(distressReports).values({
    slug,
    month,
    title,
    html,
    summaryStats: {
      totalListings,
      foreclosurePosCount: totalForeclosure,
      motivatedCount: totalMotivated,
      vtbCount: totalVtb,
      provincesReported: national.length,
      excerpt,
    },
...
export async function runMonthlyDistressReport(): Promise<{ action: string; details: string }> {
  ...
  try {
    ...
    console.log(`[distress-report] Generating monthly report...`);
