...
// DDF query terms per category. These are the coarse `contains(PublicRemarks)`
// searches used to pull candidate listings from the feed — scoring above then
// decides which candidates actually qualify. Every scanner (deal feed, monthly
// report) must query from this one list so their listing pools stay identical.
export const SEARCH_TERMS_BY_CATEGORY: Record<string, string[]> = {
  foreclosure_pos: [
    "power of sale",
    "foreclosure",
    "bank owned",
