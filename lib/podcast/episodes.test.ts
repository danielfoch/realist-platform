...
describe("mapEpisodeCta", () => {
  it("maps mortgage/rate episodes to the multiplex underwriter", () => {
    const cta = mapEpisodeCta("The Mortgage Renewal Shock Is Here");
    expect(cta.primary.href).toBe("/multiplex");
  });

  it("maps rate episodes to the multiplex underwriter", () => {
    const cta = mapEpisodeCta("Why Interest Rates Will Stay Higher For Longer");
    expect(cta.primary.href).toBe("/multiplex");
  });

  it("maps cap-rate/market episodes to listings", () => {
    const cta = mapEpisodeCta("Which Housing Market Has The Best Cap Rates?");
    expect(cta.primary.href).toBe("/listings");
  });

  it("maps a recognized city to listings with city copy", () => {
    const cta = mapEpisodeCta("Is Toronto Still Investable?");
    expect(cta.primary.href).toBe("/listings");
    expect(cta.copy).toContain("Toronto");
    expect(detectEpisodeCity("What Is Going On In Montreal?")?.city).toBe("Montreal");
  });

  it("maps power-of-sale/distress episodes to deals", () => {
    const cta = mapEpisodeCta("Power Of Sale Listings Are Surging");
    expect(cta.primary.href).toBe("/deals");
  });

  it("maps multiplex/zoning episodes to the multiplex underwriter", () => {
    const cta = mapEpisodeCta("The Fourplex Playbook For Toronto");
    expect(cta.primary.href).toBe("/multiplex");
  });

  it("defaults to listings with the conversion copy", () => {
    const cta = mapEpisodeCta("An Interview With Our Moms");
    expect(cta.primary.href).toBe("/listings");
    expect(cta.copy).toContain("pre-underwritten");
    expect(cta.secondary).toBeUndefined();
  });

  it("prefers the distress mapping over city/market when both themes appear", () => {
    const cta = mapEpisodeCta("Toronto Foreclosures Are Breaking The Market");
    expect(cta.primary.href).toBe("/deals");
  });
});
