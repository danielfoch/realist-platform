...
function directional(address:string) { return address.toUpperCase().match(/\b(E|W|N|S)\b/)?.[1] ?? null; }
function mustNotLink(a:{address:string;roll?:string;metres:number},b:{address:string;roll?:string;metres:number}) { const da=directional(a.address),db=directional(b.address); return (!!a.roll && !!b.roll && a.roll!==b.roll) || a.metres>25 || (!!da && !!db && da!==db); }
...
if (!records.some(r=>r.activityState==="paused" && r.stage==="under_review")) throw new Error("paused state fixture failed");
if (!mustNotLink({address:"123 Main St E",metres:5},{address:"123 Main St W",metres:5})) throw new Error("directional must-not-link failed");
if (!mustNotLink({address:"0 Highway 7",roll:"A",metres:5},{address:"0 Highway 7",roll:"B",metres:5})) throw new Error("parcel must-not-link failed");
console.log("paused and must-not-link: PASS");
