/**
 * content/reports/index.ts — the config-report content directory.
 *
 * TO PUBLISH A RICH REPORT: create a `<slug>-config.ts` file in this folder
 * exporting a typed `ReportContent` object (see lib/research/reportContent.ts
 * for the schema and interprovincial-migration-config.ts for a full worked
 * example), then add it to the `configReports` array below. That is the ENTIRE
 * flow — no React, no route wiring:
 *
 *   - `getConfigReport(slug)` powers /research/[slug] + its metadata.
 *   - `sortedConfigReports` drives the /research hub and generateStaticParams,
 *     newest-first.
 *
...
import { osfiDsbJune2026ConfigReport } from "./osfi-dsb-june-2026-config";
...
/** Look up a config report by slug (page route + metadata both use this). */
...
/** Every config report, newest-first — the hub and sitemap read this. */
export const sortedConfigReports: ReportContent[] =
  sortReportsNewestFirst(configReports);

/**
 * Stable alias consumed by lib/podcast/enrich.ts (related-research links).
 * Same newest-first list; kept as its own name so cross-vertical consumers
 * don't couple to the hub's export.
 */
export const reportIndex: ReportContent[] = sortedConfigReports;
