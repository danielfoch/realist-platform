...
export async function getMetaForPath(rawPath: string): Promise<PageMeta> {
  ...
  // Hosted agent results (/v/:token): the token is a private capability link —
  // resolve the page, but keep every one of them out of the index.
  if (/^\/v\/[^\/]+$/.test(path)) {
    return {
      title: "Agent result | Realist",
...
const KNOWN_APP_ROUTE_PATTERNS: RegExp[] = [
  ...
  /^\/analyses\/[^/]+\/deck$/,
  /^\/underwriting\/[^/]+$/,
  // Hosted agent result views (client/src/pages/AgentResultView.tsx).
  /^\/v\/[^/]+$/,
  /^\/admin\/events\/[^/]+\/edit$/,
  /^\/events\/[^/]+\/success$/,
