  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: "var(--rl-frame-glow)" }}
        aria-hidden="true"
      />

      <div className="overflow-hidden rounded-xl border border-border bg-card shadow-2xl ring-1 ring-border/40">
        {/* Browser chrome */}
        <div className="flex items-center gap-3 border-b border-border bg-muted/60 px-4 py-2.5">
          ...
          <div className="mx-auto flex h-6 w-full max-w-xs items-center justify-center gap-1.5 rounded-md bg-background/70 text-[11px] text-muted-foreground">
            <span className="h-2.5 w-2.5 rounded-sm border border-muted-foreground/50" aria-hidden="true" />
            realist.ca/tools/analyzer
          </div>
          <span className="w-12" aria-hidden="true" />
        </div>

        <div className="grid gap-px bg-border/60 md:grid-cols-[1.35fr_1fr]">
          {/* Left: analysis */}
          <div className="bg-card p-4 sm:p-6">
            <div className="flex items-center gap-2 rounded-lg border border-border bg-background/70 px-3 py-2">
              <Search className="h-4 w-4 shrink-0 text-muted-foreground" aria-hidden="true" />
              <span className="truncate text-sm text-foreground">24 Barton St E, Hamilton, ON</span>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2 rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-xs sm:text-sm">
              <TrendingUp className="h-4 w-4 shrink-0 text-emerald-600 dark:text-emerald-300" aria-hidden="true" />
              <span className="font-semibold text-emerald-700 dark:text-emerald-200">Strong buy-and-hold candidate</span>
              <span className="text-muted-foreground">· duplex · investor score</span>
              <span className="ml-auto font-mono text-base font-bold tabular-nums text-emerald-700 dark:text-emerald-200">84</span>
            </div>
            ...
            <dl className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
              {METRICS.map((m, i) => (
                ...
                >
                  <dt className="text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">{m.label}</dt>
                  <dd
                    className={`mt-1 font-mono text-lg font-bold tabular-nums ${m.tone === "good" ? "text-foreground" : "text-amber-600 dark:text-amber-300"}`}
                  >
                    {m.value}
                    {"unit" in m && <span className="text-xs font-medium text-muted-foreground">{m.unit}</span>}
                  </dd>
                </motion.div>
              ))}
            </dl>
            ...
            <div className="mt-4 rounded-lg border border-border bg-muted/30 p-3">
              <div className="flex items-baseline justify-between">
                <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">10-year equity</p>
                <p className="font-mono text-xs tabular-nums text-foreground/80">
                  $150k <span className="text-muted-foreground/70">→</span> $391k
                </p>
              </div>
          ...
          <div className="bg-card p-4 sm:p-6">
            <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">Assumptions</p>
            <ul className="mt-2 divide-y divide-border/60 text-sm">
              {ASSUMPTIONS.map(([k, v]) => (
                <li key={k} className="flex items-center justify-between py-1.5">
                  <span className="text-muted-foreground">{k}</span>
                  <span className="font-mono text-xs tabular-nums text-foreground sm:text-sm">{v}</span>
                </li>
              ))}
            </ul>

            <p className="mt-5 text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">Next steps</p>
            <ul className="mt-2 space-y-1.5 text-sm">
              {[
                ...
                <li key={label} className="flex items-center gap-2 rounded-md border border-border bg-muted/40 px-2.5 py-1.5 text-foreground/85">
